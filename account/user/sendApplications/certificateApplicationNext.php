<!DOCTYPE html> 
<html lang="en"> 
 
<head> 
    <meta charset="UTF-8"> 
    <meta http-equiv="X-UA-Compatible" content="IE=edge"> 
    <meta name="viewport" content="width=device-width, initial-scale=1.0"> 
    <link rel="stylesheet" href="../../css/certificates.css"> 
    <title>Выдача желищных сертификатов</title> 
</head> 
<body>
     <div class="line1"></div>
     <div class="line2"></div>
     <div class="triangle"></div>
     <div class="title">Выдача желищных сертификатов</div>
     <div class="container-max">
        <div class="container">
          <div class="words">заявление на предоставление жилищного сертификата</div>
          <div class="button">
            <input type="hidden" name="MAX_FILE_SIZE" value="30000"> 
            <label for="certificate1">прикрепить документ</label> 
            <input id="certificate1" name="application" type="file"></div>
          <div class="words">документ,удостоверяющий личность заявителя и членов семьи(супруг(а), несовершеннолетних детей)(для идентификации личности) </div>
          <div class="button">
            <input type="hidden" name="MAX_FILE_SIZE" value="30000"> 
            <label for="certificate2">прикрепить документ</label> 
            <input id="certificate2" name="application" type="file"> </div>
          <div class="words">справка о доходах за последнии шесть месяцев по трудовой и (или) предпренимательской деятельности(на всех членов семьи)</div>
          <div class="button">
            <input type="hidden" name="MAX_FILE_SIZE" value="30000"> 
            <label for="certificate3">прикрепить документ</label> 
            <input id="certificate3" name="application" type="file"> </div>
           <div class="words">справка с места работы,за исключением социально-уязвимых слоев население</div>
           <div class="button">
              <input type="hidden" name="MAX_FILE_SIZE" value="30000"> 
              <label for="certificate4">прикрепить документ</label> 
              <input id="certificate4" name="application" type="file"> </div>
            <div class="words">письмо БВУ об одобрении выдачи ипотечного займа на приобретении жилья заявителю,содержащее сведения о сумме,размере о первоначального взноса и сумме ежемесячного платежа по ипотечному жилищному займу</div>
            <div class="button">
              <input type="hidden" name="MAX_FILE_SIZE" value="30000"> 
              <label for="certificate5">прикрепить документ</label> 
              <input id="certificate5" name="application" type="file"> </div>  
          </div>
          <div class="green">отправить заявку</div>
  </div>
    <script src="certificates.js"></script>
</body>
</html>